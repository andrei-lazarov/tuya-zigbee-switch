...
void relay_on(relay_t *relay, bool fully_detached) {
  printf("relay_on\r\n");
  if (!fully_detached) {
...
void relay_off(relay_t *relay, bool fully_detached) {
  printf("relay_off\r\n");
  if (!fully_detached) {
    hal_gpio_write(relay->pin, !relay->on_high);
    if (relay->off_pin) {
...
void relay_toggle(relay_t *relay, bool fully_detached) {
  printf("relay_toggle\r\n");
  if (relay->on) {
    relay_off(relay, fully_detached);
  } else {
    relay_on(relay, fully_detached);
  }
}
